...
class OutputTransition(nn.Module):
    def __init__(self, inChans, elu, nll, numclasss):
        super(OutputTransition, self).__init__()
        self.conv1 = nn.Conv3d(inChans,numclasss, kernel_size=5, padding=2)
        self.bn1 = ContBatchNorm3d(numclasss)
        self.conv2 = nn.Conv3d(numclasss, numclasss, kernel_size=1)
        self.relu1 = ELUCons(elu, numclasss)
        if nll:
    ...
    def forward(self, x):
        ...
        out = F.sigmoid(out.contiguous())  # .permute(0, 2, 3, 4, 1)
        # out = F.softmax(out.contiguous())
        # flatten
